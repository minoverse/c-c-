...
void multiple(float arr1[SIZE][SIZE], float arr2[SIZE][SIZE], float result[SIZE][SIZE]) {
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
            result[i][j] = 0.0;
...
int main() {
    ...
    printf("Add of matrices is:\n");
    printarr(addresult);

    multiple(arr1, arr2, multipleresult);
    printf("Mutilpication of matrices is:\n");
